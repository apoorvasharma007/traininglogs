...
def test_validate_strength_session_exits_zero():
    assert main([P, RULES, str(FIXTURES_VALID / "strength_session.md")]) == 0


def test_validate_activity_session_exits_zero():
    assert main([P, RULES, str(FIXTURES_VALID / "activity_session.md")]) == 0


def test_validate_unilateral_session_exits_zero():
    assert main([P, RULES, str(FIXTURES_VALID / "unilateral_session.md")]) == 0


def test_validate_standalone_session_exits_zero():
    assert main([P, RULES, str(FIXTURES_VALID / "standalone_session.md")]) == 0


def test_validate_missing_file_exits_nonzero():
    assert main([P, RULES, "nonexistent_file.md"]) == 1


def test_validate_malformed_set_line_exits_nonzero():
    assert main([P, RULES, str(FIXTURES_INVALID / "malformed_set_line.md")]) == 1


def test_validate_malformed_goal_exits_nonzero():
    assert main([P, RULES, str(FIXTURES_INVALID / "malformed_goal.md")]) == 1
